...
2. **Create a Plan:** Once all questions are resolved, write a detailed step-by-step implementation plan that incorporates the user's answers. The plan must be self-contained and decision-complete. **Especially for simple tasks, the plan MUST always include the relevant code diffs for easy inspection before approval.** The plan file must begin with a YAML frontmatter block containing the `date` (current date) and `status` (initialized to `pending_review`).
